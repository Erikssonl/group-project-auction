...
function BidPreview({ selectedAuction, auctionDetails, handleCloseBtn, handleDeleteAuction }) {
    ...
    useEffect(() => {
        const storedPreviousBids = localStorage.getItem(`previousBids_${auctionId}`); // Hämta tidigare bud 
        if (storedPreviousBids) {
            setPreviousBids(JSON.parse(storedPreviousBids)); // Uppdatera tidigare bud 
        }

        const storedHighestBid = localStorage.getItem(`highestBid_${auctionId}`); // Hämta högsta bud
        if (storedHighestBid) {
            setRealtimeHighestBid(parseFloat(storedHighestBid)); // Uppdatera högsta bud
        }
    }, [auctionId]);

    // Effektfunktion för att uppdatera tidigare bud i lokal lagring
    useEffect(() => {
        localStorage.setItem(`previousBids_${auctionId}`, JSON.stringify(previousBids)); // Spara tidigare bud
    }, [previousBids, auctionId]);

    // Effektfunktion för att uppdatera högsta bud
    useEffect(() => {
        localStorage.setItem(`highestBid_${auctionId}`, realtimeHighestBid.toString()); // Spara högsta bud
